import { g as stripRequestBase, l as joinPaths, m as removeTrailingForwardSlash, r as collapseDuplicateSlashes, t as appendForwardSlash, u as prependForwardSlash } from "./path-4QUEXrXn.js";
import { n as matchPattern } from "./remote-BdawtaZS.js";
import { $ as isAstroError, D as MiddlewareNoDataOrNextCalled, E as LocalsReassigned, G as SessionStorageInitError, H as ReservedSlotName, K as SessionStorageSaveError, N as NoManifestAvailable, O as MiddlewareNotAResponse, Q as AstroError, T as LocalsNotAnObject, U as ResponseSentError, Z as i18nNoLocaleFoundInPath, a as CacheNotEnabled, i as AstroResponseHeadersReassigned, m as ForbiddenRewrite, n as ActionNotFoundError, o as ClientAddressNotAvailable, q as StaticClientAddressNotAvailable, r as ActionsReturnedInvalidDataError, t as ActionCalledFromServerError, z as PrerenderClientAddressNotAvailable } from "./errors-data-UhDIP5dl.js";
import { B as normalizeThePath, C as createAsyncManifestMemo, F as getErrorRoutePath, I as shouldAppendForwardSlash, M as routeHasHtmlExtension, O as getCustom404Route, R as astroToRuntimeLogger, S as readBodyWithLimit, V as pathHasLocale, _ as validateAndDecodePathname, b as SERVER_ISLAND_COMPONENT, f as copyRequest, g as MultiLevelEncodingError, h as setOriginPathname, k as getCustom500Route, m as getOriginPathname, o as getLogger, r as getRouteCache, u as getEnvironment, w as createManifestMemo, x as BodySizeLimitError, y as DEFAULT_404_ROUTE, z as normalizeTheLocale } from "./route-cache-BIo6ABSo.js";
import { a as REDIRECT_STATUS_CODES, c as fetchStateSymbol, l as originPathnameSymbol, n as ASTRO_GENERATOR, o as REROUTABLE_STATUS_CODES, t as ASTRO_ERROR_HEADER, u as responseSentSymbol$1 } from "./constants-ChO1t1dg.js";
import { parseCookie, stringifySetCookie } from "./astro_n_cookie.js";
import s from "./astro_n_piccolore.js";
import { i as defineMiddleware, n as getParams, r as getProps, t as sequence } from "./sequence-Bc8WMC59.js";
import { i as unflatten$1, r as parse, t as stringify$1 } from "./devalue-CNBmdhV5.js";
import { Br as $ZodDiscriminatedUnion, Cr as $ZodArray, Or as $ZodBoolean, gi as $ZodObject, mi as $ZodNumber, no as safeParseAsync, pi as $ZodNullable, vi as $ZodOptional, yi as $ZodPipe, zr as $ZodDefault } from "./core-CEBQcyt0.js";
import { G as isRoute404, I as renderSlotToString, K as isRoute500, a as renderPage, o as renderJSX, w as chunkToString, z as isRenderTemplateResult } from "./server-CEca8Qs4.js";
import { renderEndpoint } from "./astro_runtime_server_endpoint__js.js";
import { n as normalizeCspResourceEntry, r as pushDirective } from "./csp-DwWJD-nF.js";
import { isRenderInstruction } from "./astro_runtime_server_render_instruction__js.js";
import { i as generateCspDigest } from "./encryption-GlpTluwQ.js";
import { n as createStorage } from "./dist-DIV-On1m.js";
import { t as NOOP_MIDDLEWARE_FN } from "./noop-middleware-UtjtV7AY.js";
import { manifest } from "#astro-internal/ambient-manifest";
...
async function handlePagesWithErrorFallback(state) {
	...
	if (state.manifest.checkOrigin && isForbiddenCrossOriginRequest(ctx.request, ctx.url, ctx.isPrerendered)) return createCrossOriginForbiddenResponse(ctx.request);
	try {
...
function matchAllRoutes$1(pathname, manifest) {
...
//#endregion
//#region ../node_modules/astro/dist/core/middleware/index.js
function getMiddlewareLogger() {
	const manifest = tryGetAmbientManifest();
	if (manifest) return getLogger(manifest);
	return { warn(_label, message) {
		console.warn(message);
	} };
}
function createContext({ request, params = {}, userDefinedLocales = [], defaultLocale = "", locals = {}, clientAddress }) {
	...
	const context = {
		...
		get locals() {
			if (typeof locals !== "object") throw new AstroError(LocalsNotAnObject);
			return locals;
		},
		set locals(_) {
...
function isLocalsSerializable(value) {
	...
	while (stack.length > 0) {
		...
		if (current === null || type === "string" || type === "number" || type === "boolean") continue;
...
function trySerializeLocals(value) {
	if (isLocalsSerializable(value)) return JSON.stringify(value);
	else throw new Error("The passed value can't be serialized.");
}
//#endregion
...
async function parseFormInput(inputSchema, unparsedInput) {
	...
	return await safeParseAsync(inputSchema, input);
}
function getJsonServerHandler(handler, inputSchema) {
	return async (unparsedInput, context) => {
		...
		if (!inputSchema) return await handler(unparsedInput, context);
		const parsed = await safeParseAsync(inputSchema, unparsedInput);
		if (!parsed.success) throw new ActionInputError(parsed.error.issues);
		return await handler(parsed.data, context);
	};
}
...
function formDataToObject(formData, schema, prefix = "") {
	...
	for (const [key, baseValidator] of Object.entries(schema._zod.def.shape)) {
		...
		while (validator instanceof $ZodOptional || validator instanceof $ZodNullable || validator instanceof $ZodDefault) {
			if (validator instanceof $ZodDefault && !formDataHasKeyOrPrefix(formKeys, prefixedKey)) obj[key] = validator._zod.def.defaultValue instanceof Function ? validator._zod.def.defaultValue() : validator._zod.def.defaultValue;
			validator = validator._zod.def.innerType;
		}
		while (validator instanceof $ZodPipe) validator = validator._zod.def.in;
		...
		if (validator instanceof $ZodObject) {
			...
			else if (!(key in obj)) obj[key] = baseValidator instanceof $ZodNullable ? null : void 0;
		...
		} else if (validator instanceof $ZodArray) obj[key] = handleFormDataGetAll(prefixedKey, formData, validator);
...
function handleFormDataGetAll(key, formData, validator) {
	...
	if (elementValidator instanceof $ZodNumber) return entries.map(Number);
	else if (elementValidator instanceof $ZodBoolean) return entries.map((v) => v === "true" ? true : v === "false" ? false : Boolean(v));
	return entries;
}
function handleFormDataGet(key, formData, validator, baseValidator) {
	const value = formData.get(key);
	if (!value) return baseValidator instanceof $ZodOptional ? void 0 : null;
	return validator instanceof $ZodNumber ? Number(value) : value;
}
function unwrapBaseZ4ObjectSchema(schema, unparsedInput) {
	if (schema instanceof $ZodPipe) return unwrapBaseZ4ObjectSchema(schema._zod.def.in, unparsedInput);
...
//#endregion
export { createActionsProxy as A, isForbiddenCrossOriginRequest as B, matchAllRoutes$1 as C, finalizeI18n as D, prepareResponse as E, ALL_FETCH_FEATURES as F, computePathnameFromDomain as G, setRenderOptions as H, FetchFeatures as I, getUsedFeatures as L, isActionError as M, isInputError as N, getI18n as O, ACTION_QUERY_PARAMS as P, markFeatureUsed as R, routeComparator as S, handlePagesWithErrorFallback as T, getAmbientManifest as U, getRenderOptions as V, getSetCookiesFromResponse as W, provideSession as _, getAction as a, matchRoute as b, handleMiddlewareWithErrorFallback as c, trySerializeLocals as d, DisabledAstroCache as f, renderErrorPage as g, renderErrorFromState as h, clearActions as i, createGetActionPath as j, ActionError as k, createContext as l, clearMiddleware as m, getActionContext as n, FetchState as o, NoopAstroCache as p, serializeActionResult as r, handleMiddleware as s, defineAction as t, isLocalsSerializable as u, getRouteTable as v, handlePages as w, updateRouteTable as x, matchAllRoutes as y, createCrossOriginForbiddenResponse as z };
